...
def initialize_image_pairs(a=False):
    ...
    # (r) combine unique pairs, random flip tuples, randomize tuples in list
    image_paths = [path for path in image_paths if path not in excluded_images]
    image_pairs = list(itertools.combinations(image_paths, 2))  # all possible pairs
    image_pairs = [(a, b) if random.random() < 0.5 else (b, a) for a, b in image_pairs]
...
def smart_shuffle():
    ...
    with image_pairs_lock:
        ...
        def get_elo_difference(pair):
            return abs(elo_dict.get(pair[0], 0) - elo_dict.get(pair[1], 0)) + 0.8 * (
                count_dict.get(pair[0], 0) + count_dict.get(pair[1], 0)
            )
    ...
    return jsonify(
        {
            "image1": img1,
            "image2": img2,
            "progress": {"current": completed_pairs, "total": total_pairs},
        }
    )
...
def serve_image():
    try:
        ...
        # what about the other image types? (line 36-46) can we define MIMETYPE constants
        # Handle jpg/jpeg special case
        if file_extension in ["jpg", "jpeg"]:
            mimetype = "image/jpeg"
        elif file_extension in ["png", "gif", "webp"]:
...
def autosave_rankings():
    ...
    # Save rankings
    rankings = elo_ranking.get_rankings()
    rankings_filename = os.path.join(
        current_directory, f"image_rankings_autosave_{current_date}.csv"
    )
    ...
    comparisons_filename = os.path.join(
        current_directory, f"comparisons_autosave_{current_date}.csv"
    )
    ...
    app.logger.info(
        f"Autosave completed. Files saved in {current_directory}: {os.path.basename(rankings_filename)}, {os.path.basename(comparisons_filename)}"
    )
...
def remove_image():
    ...
    global image_pairs
    image_pairs = [
        (img1, img2) for img1, img2 in image_pairs if img1 != image and img2 != image
    ]
...
def get_progress():
    return jsonify({"current": current_pair_index, "total": len(image_pairs)})
...
def set_directory():
    ...
    try:
        ...
        if not os.path.isdir(directory):
            ...
            return (
                jsonify({"success": False, "error": f"Not a directory: {directory}"}),
                400,
            )
    ...
    try:
        ...
        if not rankings:
            ...
            return (
                jsonify(
                    {
                        "error": "No rankings data available. Please make some comparisons first."
                    }
                ),
                400,
            )
    ...
    try:
        ...
        if not comparisons:
            ...
            return (
                jsonify(
                    {
                        "error": "No comparisons data available. Please make some comparisons first."
                    }
                ),
                400,
            )
...
def import_comparison_history():
    ...
    image_pairs = [
        ...
        if img1 not in losers_to_remove and img2 not in losers_to_remove
    ]
    ...
    image_pairs = [
        (img1, img2)
        for img1, img2 in image_pairs
        if (img1, img2) not in pairs_to_remove
    ]
...
if __name__ == "__main__":
    initialize_image_pairs()
    app.run(debug=False, threaded=True, port=4999)
